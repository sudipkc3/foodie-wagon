...
export function ReportsPage() {
  ...
  const report = useMemo(() => {
    ...
    const waste = state.waste.filter((entry) => dayKey(entry.at) >= from).reduce((sum, entry) => sum + entry.cost, 0)
    const whatsapp = inRange.flatMap((order) => order.whatsapp)
    return { inRange, valid, cancelled, gross, daily, cakes, staffRow, hours, labour, waste, whatsapp }
  }, [state, days])
  ...
  return (
    <div className="space-y-6">
      <PageHeading eyebrow="Reports & analytics" title="Know what is moving" description="Sales, sources, fulfilment, products, team performance, communication and costs — computed from live data."
